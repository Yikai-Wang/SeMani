...
from dalle_pytorch import DALLE_CONTEXT
...
from dalle_pytorch.val_engine import validation_finetune
...
parser.add_argument('--val_folder', type=str, default = "/home/work/user-job-dir/data/CUB-200",
...
parser.add_argument('--text_on_visual_context', action='store_true', help = '')
...
model_group.add_argument('--loss_vc_weight', default = 1, type = int, help = 'Image loss weight')
...
assert len(dataset_type_list) == len(text_folder_list) and len(dataset_type_list) == len(image_folder_list), "length of list of dataset, text_folder, image_folder is not the same"
...
LOSS_VC_WEIGHT = args.loss_vc_weight
...
args.name = "RESUME_" * RESUME + 'SCRATCH_' * (1 - RESUME) + \
            ...
            'TxtOnVC_' * args.text_on_visual_context + \
            'ViCo{}_'.format(LOSS_VC_WEIGHT) + \
...
def get_dataset(dataset_name, text_folder, image_folder):
    ...
    if dataset_name == 'cub':
        ds = CUB_TextAndImageDataset(  # TextImageDataset(
            text_folder,
            image_folder,
            phase="training",
...
if RESUME:
    ...
    dalle_params.update({"loss_vc_weight": LOSS_VC_WEIGHT})
...
# modify the preprocess
clip_img_preprocess = T.Compose(
    # Suppose that the image has been "RGB" format and normalized to [0, 1]
...
def save_model(path, epoch, global_step):
    ...
    save_obj = {
        ...
        'text_on_visual_context': args.text_on_visual_context
    }
...
for epoch in range(resume_epoch, EPOCHS):
    ...
    for i, (text, images, original_text, image_name) in enumerate(training_loader):
        ...
        not_use_context_id = list(set(range(bsz)) - set(use_context_id))
        ...
        if images.dim() == 4:
            ...
            txt_seq = text.shape[-1]
            seq_len = vc_seq + 2 + txt_seq + vc_seq - 1
            visual_context_mask_0pad = ~torch.ones(seq_len, seq_len).triu_(seq_len - seq_len + 1).bool().to(text.device)
            visual_context_mask_0pad[vc_seq+1: vc_seq+1+txt_seq, :vc_seq+1] = 0
            # For no context training
            visual_context_mask_0pad = visual_context_mask_0pad.unsqueeze(0).repeat(bsz, 1, 1)
            visual_context_mask_0pad[not_use_context_id, vc_seq+1+txt_seq:, :vc_seq+1] = 0

            if args.text_on_visual_context:
                transformer_attn_kwargs = {}
            else:
                transformer_attn_kwargs = {"mask": visual_context_mask_0pad}

            if args.fp16:
                images = vae.get_codebook_indices(images.half())
            else:
        ...
        loss, (loss_text, loss_vc, loss_image), loss_meaningful_text, img_logits = \
            distr_dalle(
                text, images, visual_context, use_context,
                transformer_attn_kwargs=transformer_attn_kwargs, return_loss=True
            )
        ...
        if WORLD_SIZE > 1:
            ...
            iter_loss_vc.update(reduce_all_tensor(loss_vc).item())
            ...
            iter_loss_vc.update(loss_vc.item())
        ...
        epoch_loss_vc.update(iter_loss_vc.val)
        ...
        if global_iter % 50 == 0 and ga_step_cnt == 0:
            if IS_LOG:
                ...
                print(
                    ...
                    'Loss_Visual_Context: {:.4f}, Loss_Meaningful_Text: {:.4f}, Loss_CLIP: {:.4f}'.format(
                        datetime.now(), epoch, i+1, len(training_loader), iter_loss.avg,
                        iter_loss_text.avg, iter_loss_image.avg,
                        iter_loss_vc.avg,
                        iter_loss_meaningful_text.avg,
                        iter_loss_clip.avg
                    )
                )
                ...
                writer.add_scalar("Training_Step/loss_visual_context", iter_loss_vc.avg, global_iter)
    ...
    val_loss, (val_loss_text, val_loss_vc, val_loss_image), val_loss_meaningful_text, val_loss_clip = \
        validation_finetune(distr_dalle, val_loader, args, WORLD_SIZE, vae, clip_model, clip_img_preprocess)
    torch.cuda.synchronize()
    ...
    if IS_LOG:
        ...
        writer.add_scalar("Training_Epoch/loss_visual_context", epoch_loss_vc.avg, epoch)
        ...
        print(
            ...
            'Loss_Visual_Context: {:.4f}, Loss_Meaningful_Text: {:.4f}, Loss_CLIP: {:.4f}, '
            'consuming {:.2f} s.'.format(
                datetime.now(), epoch, val_loss, val_loss_text, val_loss_image,
                val_loss_vc, val_loss_meaningful_text, val_loss_clip,
        ...
        writer.add_scalar(f"Val_Epoch_on_{val_name}/loss_visual_context", val_loss_vc, epoch)
