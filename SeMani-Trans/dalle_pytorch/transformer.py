...
#         maxes = x.max(dim = self.dim, keepdim = True)[0]
#         return x / maxes
...
#         super().__init__()
#         self.norm = StableLayerNorm(dim)  # nn.LayerNorm(dim)
#         self.fn = fn
#
#     def forward(self, x, **kwargs):
...
class SandwichNorm(nn.Module):
    def __init__(self, dim, fn):
        super().__init__()
        self.pre_norm = StableLayerNorm(dim)  # nn.LayerNorm(dim)
        self.fn = fn
        self.post_norm = StableLayerNorm(dim)
        ...
        *,
        ...
        for ind, sparse_attn, attn_type in zip(range(depth), sparse_layer, attn_type_layer):
            ...
            layers.append(nn.ModuleList([
                LayerScale(dim, ind + 1, SandwichNorm(dim, attn)),
                LayerScale(dim, ind + 1, SandwichNorm(dim, FeedForward(dim, mult = ff_mult, dropout = ff_dropout)))
            ]))

        execute_type = ReversibleSequence if reversible else SequentialSequence  # addition here
